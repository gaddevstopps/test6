...
st.title("Ad-Spending Business Finder (Auto Location Enabled)")
...
def get_lat_lng(city_query, api_key):
    search = GoogleSearch({
        "engine": "google",
        "q": city_query,
        "api_key": api_key
    ...
    if "knowledge_graph" in result and "latitude" in result["knowledge_graph"]:
        lat = result["knowledge_graph"]["latitude"]
        lng = result["knowledge_graph"]["longitude"]
...
if business_type and city:
    ...
    ll_value = get_lat_lng(city, api_key)
    if not ll_value:
        st.error("Could not determine location coordinates. Try a more specific city name.")
    else:
        ...
        while has_next:
            ...
            st.write(results)  # Optional debug output
